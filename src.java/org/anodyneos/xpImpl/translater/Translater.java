...
public class Translater extends BaseParser {
    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            resolver.addProtocolHandler("classpath", new ClassLoaderURIHandler(Thread.currentThread()
    ...
    public static TranslaterResult translate(String xpRoot, String xpPage, String javaFile, String registryFile,
            EntityResolver er) throws Exception {
        OutputStream os;
    ...
    /**
     * @param xpRoot -
     *            The physical directory where the xp are located
     * @param tempRoot -
     *            The physical directory where the java file is to be based
     * @param xpURI -
     *            The URI of the desired xp
     * @param registryFile -
     *            RegistryFile of TLDs
     * @param resolver -
     *            UnifiedResolver
    ...
    public TranslaterResult translate(String tempRoot, URI xpURI, TagLibraryRegistry registry, UnifiedResolver resolver)
        ...
        try {
            ...
            String className = getClassName(xpURI); // i.e.
                                                    // xp.WEB_INF.common.header
            ...
            // now translate the dependents (There is no check to see if they
            // are out of date)
     ...
     * @return
     */
    public static boolean xpNeedsTranslating(URI xpURI, String tempRoot, UnifiedResolver resolver) throws XpException {
     ...
     * @return
     */
    public static boolean xpNeedsCompiling(URI xpURI, String tempRoot, UnifiedResolver resolver) throws XpException {
    ...
    public static String getJavaFile(String tempRoot, URI xpURI) {
        ...
        fullPath = fullPath.replace('-', '_'); // TODO replace with more robust
                                                // replacement
        fullPath = fullPath.replace('.', '_'); // TODO replace with more robust
                                                // replacement
        fullPath += ".java";
    ...
    /**
     *
     * Convert webapp:///WEB-INF/xp/account/account_list.query.xp into
     * xp.WEB_INF.xp.account.account_list_query
     *
     * @param xpURI
     * @return String
     */
        ...
        if (pathPrefix == null) {
            ...
            if (pathPrefix.endsWith("/")) {
                if (pathSuffix.startsWith("/")) {
                    // if the prefix ends in / and the suffix begins with / then
                    // strip the / from the suffix
                    pathSuffix = pathSuffix.substring(1, pathSuffix.length());
                }
            } else {
                if (!pathSuffix.startsWith("/")) {
                    // if the prefix does NOT end in / and the suffix does NOT
                    // begin with / then append a / to c
                    c = c.concat("/");
                }
            }
